...
resource "aws_subnet" "public" { # first name is public[0], second name is public[1]
  ...
  tags = merge(
    ...
    {
        Name = "${local.resource_name}-public-${local.az_names[count.index]}" #This is Interpolation (variables + text)
    }
  )
}
...
resource "aws_subnet" "private" { # first name is public[0], second name is public[1]
  ...
  tags = merge(
    ...
    {
        Name = "${local.resource_name}-private-${local.az_names[count.index]}" #This is Interpolation (variables + text)
    }
  )
}
...
resource "aws_subnet" "database" { # first name is public[0], second name is public[1]
  ...
  tags = merge(
    ...
    {
        Name = "${local.resource_name}-database-${local.az_names[count.index]}" #This is Interpolation (variables + text)
    }
  )
}
